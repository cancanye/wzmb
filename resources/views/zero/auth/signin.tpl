	...
	<body id="kt_body" class="app-blank app-blank bgi-size-cover bgi-position-center bgi-no-repeat">
						...
						<div class="card-body p-10 p-lg-20">							
							<form class="form w-100" novalidate="novalidate" id="kt_sign_in_form" data-kt-redirect-url="/user/dashboard" action="#">								
								...
								<div class="fv-row mb-8">
									<select class="" id="signin-email" type="email" name="email" placeholder="{$trans->t('email')}" autocomplete="new-password" />
									<option></option>
									{foreach $email_suffixs as $suffix}
										<option value="{$suffix}">@{$suffix}</option>
									{/foreach}
									</select>
								</div>
								<div class="fv-row mb-8">
									<input type="password" placeholder="{$trans->t('passwd')}" name="password" autocomplete="off" id="signin-passwd" data-kt-translate="sign-in-input-password" class="form-control form-control-lg bg-transparent" autocomplete="new-password" />
								</div>
		...
		<script>
			$(document).ready(function() {
				var suffixs = {json_encode($email_suffixs)};
